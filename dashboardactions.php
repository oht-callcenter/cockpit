...
$sqlA = "SELECT * FROM Actions WHERE trkfix_action_cat = 'Dashboard'";
...
while ($actions = mysqli_fetch_array($queryA)) {
    ...
    if (isset($_SESSION['role'])) {
        if ($_SESSION['role'] == 3) {
            echo "<a href='#' id='delactionDSHBRD' class='button tiny alert'>Delete</a>";
        }
    }
    //echo "<a href='#' id='delactionDSHBRD' class='button tiny alert'>Delete</a>";
...
<script>
    $(document).one('click', '#delactionDSHBRD', function() {
